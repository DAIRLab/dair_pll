...
StudyParams = Tuple[bool,
                    Optional[MeshRepresentation],
                    Optional[MultibodyLosses],
                    str]
DEEP_LEARNED_STUDY_PARAMS = (False, None, None, 'MLP')
DEEP_DIFF_PHYS_STUDY_PARAMS = (True,
                               MeshRepresentation.DEEP_SUPPORT_CONVEX,
                               MultibodyLosses.PREDICTION_LOSS,
                               'DEEP_DIFF_PHYS')
DEEP_CONTACTNETS_STUDY_PARAMS = (True,
                                 MeshRepresentation.DEEP_SUPPORT_CONVEX,
                                 MultibodyLosses.CONTACTNETS_ANITESCU_LOSS,
                                 'DEEP_CONTACTNETS')
STUDY_PARAMS = cast(List[StudyParams], [
...
def main(sweep_num: int) -> None:
    ...
    deep_learnable_config = DeepLearnableSystemConfig(
        ...
        integrator_type=DeltaVelocityIntegrator
    )
    ...
    for study_params in STUDY_PARAMS:
        if study_params[0]:
            default_experiment_config = deepcopy(
                multibody_experiment_config)
