...
class SetpointFromTargetSystem(LeafSystem):
    ...
    def __init__(
        ...
        robot_id: ModelInstanceIndex,
        fingertip_body_names: List[str],
        traj_time_len: float = 1.0,
    ):
        super().__init__()
        ...
        self._target_kinematics_input_port = self.DeclareAbstractInputPort(
            "lcmt_fingertips_target_kinematics",
            Value(lcmt_fingertips_target_kinematics()),
        )
        ...
        self._trajectory_index = self.DeclareAbstractState(Value(PiecewisePolynomial()))
    ...
    def calc_setpoint(self, context, robot_setpoint):
        ...
        if traj.get_number_of_segments() < 1:
            robot_setpoint.get_mutable_value()[:] = self.EvalVectorInput(
                context, self._robot_state_input_port.get_index()
            ).get_value()[:]
        else:
            n_pos = robot_setpoint.get_mutable_value()
            robot_setpoint.get_mutable_value()[: robot_setpoint.size() // 2] = (
                traj.value(time).flatten()
            )
            robot_setpoint.get_mutable_value()[robot_setpoint.size() // 2 :] = (
                traj.derivative().value(time).flatten()
            )
    ...
    def poll_lcm_target(self, context, state_next):
        ...
        target_timestamp = context.get_mutable_discrete_state(
            self._prev_target_timestamp_idx
        )
        ...
        cur_state = self.EvalVectorInput(
            context, self._robot_state_input_port.get_index()
        ).get_value()
        ...
        if target_timestamp[0] >= 0:
            for finger_idx, state_idx in enumerate(
                finger_idx_from_body_name(self._plant, self._robot_id, self._body_names)
            ):
                pos_idx = 3 * state_idx
                vel_idx = len(target_state) // 2 + pos_idx
                target_state[pos_idx : pos_idx + 3] = target_kinematics.targetPos[
                    3 * finger_idx : 3 * finger_idx + 3
                ]
                target_state[vel_idx : vel_idx + 3] = target_kinematics.targetVel[
                    3 * finger_idx : 3 * finger_idx + 3
                ]
            # If relative to current state:
            if not target_kinematics.isAbsoluteTargetPos:
                target_state[: len(target_state) // 2] += cur_state[
                    : len(target_state) // 2
                ]
        ...
        samples = np.vstack(
            [
                cur_state[: len(target_state) // 2],
                target_state[: len(target_state) // 2],
            ]
        ).T
        samples_dot = np.vstack(
            [
                cur_state[len(target_state) // 2 :],
                target_state[len(target_state) // 2 :],
            ]
        ...
        state_next.get_mutable_discrete_state().set_value(
            np.array([target_timestamp[0]])
        )
...
class DensetactIOSystem(LeafSystem):
    ...
    def __init__(
        ...
        self._avg_contact_input_port = self.DeclareAbstractInputPort(
            ...
            Value({"force": dict(), "point": dict(), "normal": dict()}),
        )
        self._robot_state_input_port = self.DeclareVectorInputPort(
            "robot_state", BasicVector(self._plant.num_multibody_states(self._robot_id))
        )
    ...
    def calc_densetact_output(self, context, densetact_msg):
        ...
        for fingertip_idx, body_name in zip(
            ...
            fingertip_pose_W = np.array(
                [
                    robot_state[fingertip_idx * 3],
                    robot_state[fingertip_idx * 3 + 1],
                    robot_state[fingertip_idx * 3 + 2],
                ]
            )
    ...
    def rotation_matrix_from_vectors(vec1, vec2):
        ...
        """
        vec1_norm, vec2_norm = (vec1 / (np.linalg.norm(vec1))).reshape(3), (
...
class FingerTipIOSystem(LeafSystem):
    ...
    def __init__(
        self,
        plant: MultibodyPlant,
        object_id: ModelInstanceIndex,
        fingertip_body_names: List[str],
        fingerpos_std: float = 0.0,
    ):
        super().__init__()
        ...
        self._body_vels_port = self.DeclareAbstractInputPort(
            "plant_body_velocities",
            plant.get_body_spatial_velocities_output_port().Allocate(),
        )
    ...
    def calc_fingertips_position_output(self, context, fingertips_positions_msg):
        ...
        for body_enum, body_name in enumerate(self._fingertip_body_names):
            ...
            for idx in range(3):
                fingertips_positions_msg.get_mutable_value().curPos[
                    body_enum * 3 + idx
                ] = body_poses[body_idx].translation()[idx] + self._rng.normal(
                    0.0, self._pose_std
                )
                fingertips_positions_msg.get_mutable_value().curVel[
                    body_enum * 3 + idx
                ] = body_vels[body_idx].translational()[idx]
    ...
    def calc_object_output(self, context, object_msg):
        ...
        for idx in range(self._object_nq):
            ...
            object_msg.get_mutable_value().position_names.append(
                self._object_state_names[idx]
            )
        for idx in range(self._object_nv):
            object_msg.get_mutable_value().velocity.append(
                state.GetAtIndex(self._object_nq + idx)
            )
            object_msg.get_mutable_value().velocity_names.append(
                self._object_state_names[self._object_nq + idx]
            )
...
def sim_diagram_builder(
    ...
    fingertip_pos_pub = builder.AddSystem(
        LcmPublisherSystem(
            lcm_channels["fingertips_position"],
            PySerializer(lcmt_fingertips_position),
            lcm,
            lcm_pub_dt,
        )
    )
    object_state_pub = builder.AddSystem(
        LcmPublisherSystem(
            lcm_channels["object_state"],
            PySerializer(lcmt_object_state),
            lcm,
            lcm_pub_dt,
        )
    )
    ...
    builder.Connect(
        ...
        fingertip_io_system.GetInputPort("plant_body_poses"),
    )
    builder.Connect(
        plant.get_body_spatial_velocities_output_port(),
        fingertip_io_system.GetInputPort("plant_body_velocities"),
    )
    builder.Connect(
        fingertip_io_system.GetOutputPort("lcmt_fingertips_position"),
        fingertip_pos_pub.get_input_port(),
    )
    builder.Connect(
        plant.get_state_output_port(object_model_id),
        fingertip_io_system.GetInputPort("object_state"),
    )
    builder.Connect(
        fingertip_io_system.GetOutputPort("lcmt_object_state"),
    ...
    densetact_pub = builder.AddSystem(
        LcmPublisherSystem(
            lcm_channels["densetact"],
            PySerializer(lcmt_densetact_measurement_data),
            lcm,
            lcm_densetact_dt,
        )
    )
    builder.Connect(
        cf_averager.get_output_port(),
        densetact_io_system.GetInputPort("averaged_contact_data"),
    )
    builder.Connect(
        plant.get_state_output_port(robot_model_id),
        densetact_io_system.GetInputPort("robot_state"),
    )
    builder.Connect(
        densetact_io_system.GetOutputPort("lcmt_densetact_measurement_data"),
        densetact_pub.get_input_port(),
    )

    # PID Robot Controller
    fingertips_target_sub = builder.AddSystem(
        LcmSubscriberSystem(
            lcm_channels["fingertips_target"],
            PySerializer(lcmt_fingertips_target_kinematics),
            lcm,
        )
    )
    fingertips_target_to_setpoint = builder.AddSystem(
        SetpointFromTargetSystem(plant, robot_model_id)
    )
    control_size = plant.get_actuation_input_port(robot_model_id).size()
    pid_controller = builder.AddSystem(
        PidController(
            pid_gains[0] * np.ones(control_size),
            pid_gains[1] * np.ones(control_size),
            pid_gains[2] * np.ones(control_size),
        )
    )
    builder.Connect(
        fingertips_target_sub.get_output_port(),
        fingertips_target_to_setpoint.GetInputPort("lcmt_fingertips_target_kinematics"),
    )
    builder.Connect(
        plant.get_state_output_port(robot_model_id),
        fingertips_target_to_setpoint.GetInputPort("robot_state"),
    )
    builder.Connect(
        plant.get_state_output_port(robot_model_id),
    ...
    builder.Connect(
        fingertips_target_to_setpoint.get_output_port(),
        pid_controller.get_input_port_desired_state(),
    )
    builder.Connect(
        pid_controller.get_output_port_control(),
        plant.get_actuation_input_port(robot_model_id),
    )
...
@gin.configurable
def main(
    init_sim_state: Optional[Dict[str, List[float]]] = None, sim_rate: float = 1.0
):
    """Main function for simulation"""
    print("Simulation With Object and Robot")

    plant_diagram = MultibodyPlantDiagram()
    plant_context = plant_diagram.plant.GetMyMutableContextFromRoot(
    ...
    if init_sim_state is not None:
        for model_name, model_state in init_sim_state.items():
            ...
            plant_diagram.plant.SetPositionsAndVelocities(
                plant_context, model_id, np.array(model_state)
            )
