...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        init_param = init_data_dict["learned"]['_multibody_terms.contact_terms.geometries.3._vertices_param'].detach().cpu().numpy().T
        ...
        init_param = 0.005 * np.random.rand(3, 20)
    ...
    final_param = data_dict["learned"]['_multibody_terms.contact_terms.geometries.3._vertices_param'].detach().cpu().numpy().T
    key_base = '_learned_trajectory._trajectories_q0'
    key_idx = 0
    ...
    # from d8_5cm_mmscale.obj
    ground_vertex_raw = np.array([[0., -0.025, 0.],
                                    [0., 0., -0.025],
                                    [0.025, 0., 0.],
                                    [0., 0., 0.025],
                                    [0., 0.025, 0.],
    ...
    true_geom = Convex(ground_vertex_raw.T, "True")
    radius = 0.01
    ...
    meshcat.SetObject("/learned", Convex(init_param, "Learned"), Rgba(0.0, 0.0, 0.8, 0.7))
    ...
    for idx in range(120):
        ...
        meshcat.SetObject("/learned", Convex(current_param, "Learned"), Rgba(0.0, 0.0, 0.8, 0.7))
        meshcat.SetTransform(
                        "/learned",
