...
from dair_pll.multibody_learnable_system import (
    LOSS_INERTIA_AGNOSTIC,
    LOSS_PLL_ORIGINAL,
    LOSS_BALANCED,
    LOSS_POWER,
    LOSS_CONTACT_VELOCITY,
    LOSS_VARIATIONS,
...
RUN_PREFIX_TO_FOLDER_NAME = {
    "tc": f"{TEST}_{CUBE_SYSTEM}",
...
WANDB_PROJECTS = {True: WANDB_PROJECT_LOCAL, False: WANDB_PROJECT_CLUSTER}
...
WANDB_NO_GROUP_MESSAGE = 'echo "Not exporting WANDB_RUN_GROUP since restarting."'
...
GRAVITY_FRACTIONS = [0.0, 0.5, 1.0, 1.5, 2.0]
...
def create_instance(
    ...
    if not structured:
        ...
        if not contactnets:
            print(
                "Must use prediction loss with end-to-end model --> "
                + "setting to prediction loss."
            )
            contactnets = False
        if not regenerate:
            print(
                "Can't regenerate URDFs from end-to-end model --> " + "no regeneration."
            )
            regenerate = False
    ...
    if system == ASYMMETRIC_SYSTEM and geometry == BOX_TYPE:
        print("No box representation of asymmetric system --> " + "using polygon.")
        geometry = POLYGON_TYPE

    print(f"Generating experiment {storage_folder_name}/{run_name}")

    if wandb_group_id is None:
        wandb_group_id = "" if restart else f"{run_name}_{wandb.util.generate_id()}"
    ...
    if not restart:
        ...
        train_options += (
            f" --additional-forces={additional_forces}"
...
def attach_tb(name: str, local: bool = False):
    ...
    if not local:
        ...
        tboard_cmd = [
            "sbatch",
            f"--output={tb_logfile}",
            f"--job-name=tb_{name}".format(tb_logfile),
            tb_script,
            op.join(git_folder, "results", name, "tensorboard"),
            name,
        ]
...
def take_care_of_file_management(
    overwrite: str, storage_name: str, run_name: str
) -> None:
...
def check_for_git_updates(repo):
    ...
    if commits_ahead > 0:
        if not click.confirm(
            f"You are {commits_ahead} commits ahead of" + f" main branch, continue?"
        ):
...
def experiment_class_command(
    ...
    w_res: float,
    w_res_w: float,
    dataset_exponent: int = None,
    last_run_num: int = None,
    number: int = 1,
    do_residual: bool = False,
    additional_forces: str = None,
    g_frac: float = 1.0,
    ...
    def get_run_name_pattern(category, system):
        run_name_pattern = (
            "i"
            if category == HYPERPARAMETER_REAL
            else (
                "v"
                if (category == SWEEP and additional_forces == VORTEX_AUGMENTATION)
                else (
                    "b"
                    if (category == SWEEP and additional_forces == VISCOUS_AUGMENTATION)
                    else "a" if category == SIM_AUG_HYPERPARAMETER else category[0]
                )
            )
        )  # t/d/h/i/v/b for test/dev/{hp sim/real}/vortex/viscous
        run_name_pattern += system[0]  # c for cube or e for elbow
        run_name_pattern += (
            "????"
            if category == HYPERPARAMETER_SIM
            else (
                "????"
                if category == HYPERPARAMETER_REAL
                else "????" if category == SIM_AUG_HYPERPARAMETER else "??"
            )
        )
    ...
    storage_folder_name += (
        f"_{additional_forces}"
        if not additional_forces in [None, GRAVITY_AUGMENTATION]
        else ""
    )
    ...
    if run_name is None:
        nums_to_display = (
            ...
            if category == HYPERPARAMETER_SIM
                ...
                4
                if category == HYPERPARAMETER_REAL
                else 4 if category == SIM_AUG_HYPERPARAMETER else 2
            )
        )
        ...
        run_name += (
            "c" if system == CUBE_SYSTEM else "e" if system == ELBOW_SYSTEM else "a"
        )
    ...
    if op.isdir(op.join(storage_name, "runs", run_name)):
        ...
        raise RuntimeError(
            f"Found experiment run {storage_folder_name}/"
                ...
                if category == HYPERPARAMETER_SIM
                    ...
                    if category == HYPERPARAMETER_REAL
                        ...
                        if category == GRAVITY_SWEEP
                            ...
                            64
                            if category == SIM_AUG_HYPERPARAMETER
                            else 2**dataset_exponent
                        )
                    )
                )
            )
    ...
    source = (
        SIM_SOURCE
        if (category == SWEEP and additional_forces != None)
        else (
            REAL_SOURCE
            if category == SWEEP
            else REAL_SOURCE if category == HYPERPARAMETER_REAL else SIM_SOURCE
        )
    )

    names = [run_name] if number == 1 else [f"{run_name}-{i}" for i in range(number)]
    wandb_group_id = None if number == 1 else f"{run_name}_{wandb.util.generate_id()}"
...
@click.option(
    "--number", default=1, help="number of grouped identical experiments to run"
)
@click.option(
...
@cli.command("restart")
@click.argument("run_name")
@click.option("--storage-folder-name", type=str, default="")
@click.option(
    "--local/--cluster",
    default=False,
    help="whether running script locally or on cluster.",
...
def restart_command(run_name: str, storage_folder_name: str, local: bool):
    ...
    if not op.isdir(op.join(storage_name, "runs", run_name)):
        raise RuntimeError(
            f"Error!  Could not find run under "
...
@cli.command("sweep")
@click.option("--sweep-name", type=str, default=None)
@click.option(
    "--number", default=1, help="number of grouped identical experiments to run"
)
@click.option(
...
@click.option(
    "--min-exponent", type=int, default=2, help="minimum dataset exponent to use"
)
def sweep_command(
    sweep_name: str,
    number: int,
    ...
    if additional_forces == GRAVITY_AUGMENTATION:
        print(
            "Gravity augmentation --> sweeping over gravity instead of "
            + "dataset size."
        )
    ...
    if (category == GRAVITY_SWEEP) or (additional_forces == None):
        ...
        partial_storage_name = op.join(
            repo_dir, "results", f"{category}_{system}_{additional_forces}"
        )

    sweep_range = range(2, 10) if category == SWEEP else range(len(GRAVITY_FRACTIONS))
    ...
    W_RES_BY_LOSS_VAR = {3: 0.001, 1: 1, 0: 1.0}
    W_RES_W_BY_LOSS_VAR = {3: 0.0, 1: 0.1, 0: 0.0}
    ...
    print(
        f"Using hyperparameters: ({w_pred}, {w_comp}, {w_diss}, {w_pen}, {w_res}, {w_res_w})"
    )
...
@click.option("--hp_name", type=str, default=None)
@click.option(
    "--number", default=1, help="number of grouped identical experiments to run"
)
@click.option(
