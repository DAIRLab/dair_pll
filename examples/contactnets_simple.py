...
def main(
    ...
    if structured:
        ...
        learnable_config = MultibodyLearnableSystemConfig(
            urdfs=urdfs,
            loss=loss,
            inertia_mode=int(inertia_params),
    ...
    # Make experiment.
    experiment = (
        DrakeMultibodyLearnableExperiment(experiment_config)
        if structured
        else DrakeDeepLearnableExperiment(experiment_config)
    )
    ...
    if simulation:
        ...
        if additional_forces == None:
            ...
            data_generation_system = experiment.get_augmented_system(additional_forces)
        ...
        file_utils.import_data_to_storage(
            storage_name, import_data_dir=import_directory, num=dataset_size
        )
