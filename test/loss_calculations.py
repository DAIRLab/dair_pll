...
def sophter_loss(x: Tensor, u: Tensor, x_plus: Tensor):
    ...
    def normal_mat_pad(x):
        return SoPhTERTensorUtils.diag_prepend(x, 0, k)

    def normal_vec_pad(x):
        return SoPhTERTensorUtils.pad_left(x, 0, k)

    def tangent_mat_pad(x):
        return SoPhTERTensorUtils.diag_append(x, 0, 2 * k)

    def tangent_vec_pad(x):
        return SoPhTERTensorUtils.pad_right(x, 0, 2 * k)

    # lambda_n and phi complementarity
    comp_n_A = config.w_comp_n * SoPhTERTensorUtils.matrix_diag(
        (phi_plus.squeeze(2) ** 2)
    )
    comp_n_A = tangent_mat_pad(comp_n_A)

    # lambda_t and phi complementarity
    phi_expand = E_2.bmm(phi_plus)
    comp_t_A = config.w_comp_t * SoPhTERTensorUtils.matrix_diag(
        (phi_expand.squeeze(2) ** 2)
    )
    ...
    try:
        full_sol = self.qcqp(
            2 * A, b.transpose(1, 2), torch.rand(b.transpose(1, 2).size()), 1e-10, 10000
        )
    ...
    contact_mask = (
        torch.norm(F_data, 2, dim=1).unsqueeze(2) > config.w_contact_threshold
    )
    ...
    loss_terms = [
        utils.compute_quadratic_loss(comp_n_A, b_zero, c_zero, full_sol).sum(),
    ...
    def phi_penalizer(phi):
        return torch.sum(torch.clamp(-phi, min=0) ** 2)
    ...
    st_pen_loss = config.w_st_estimate_pen * torch.sum(
        torch.clamp(-phi_plus_hat, min=0) ** 2
    )  # /batch_n
...
def pll_loss(
    x: Tensor, u: Tensor, x_plus: Tensor
