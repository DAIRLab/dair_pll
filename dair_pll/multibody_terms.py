...
def init_symbolic_plant_context_and_state(
    ...
    v = MakeVectorVariable(plant.num_velocities(), "v", Variable.Type.CONTINUOUS)
    x = np.concatenate([q, v], axis=-1)

    # Set :py:class:`StateSpace` symbolic state inside
    DrakeStateConverter.state_to_context(
        plant, context, x, plant_diagram.model_ids, space
    )
...
class LagrangianTerms(Module):
    ...
    def __init__(
        ...
        plant, context, q, v = init_symbolic_plant_context_and_state(plant_diagram)
        ...
        u = MakeVectorVariable(plant.num_actuated_dofs(), "u", Variable.Type.CONTINUOUS)
        drake_forces_expression = (
            -plant.CalcBiasTerm(context)
            + plant.MakeActuationMatrix() @ u
        ...
        for body_param_tensor, body in zip(body_param_tensors, bodies):
            ...
            body_parameter = [
                Parameter(
                    body_param_tensor[0],
                    requires_grad=(
                        learn_body and learnable_body_dict[body.name()].inertia_mass
                    ),
                ),
                Parameter(
                    body_param_tensor[1:4],
                    requires_grad=(
                        learn_body and learnable_body_dict[body.name()].inertia_com
                    ),
                ),
                Parameter(
                    body_param_tensor[4:],
                    requires_grad=(
                        learn_body
                        and learnable_body_dict[body.name()].inertia_moments_products
                    ),
                ),
            ]
    ...
    def extract_body_parameters_and_variables(
        ...
        """
        all_bodies, all_body_ids = drake_utils.get_all_inertial_bodies(plant, model_ids)

        body_parameter_list = []
        body_variable_list = []
        for body, body_id in zip(all_bodies, all_body_ids):
            mass = Variable(f"{body_id}_m", Variable.Type.CONTINUOUS)
            p_BoBcm_B = MakeVectorVariable(
                CENTER_OF_MASS_DOF, f"{body_id}_com", Variable.Type.CONTINUOUS
            )
            I_BBcm_B = MakeVectorVariable(
                INERTIA_TENSOR_DOF, f"{body_id}_I", Variable.Type.CONTINUOUS
            )
            ...
            body_spatial_inertia = SpatialInertia_[Expression].MakeFromCentralInertia(
                mass=mass,
                p_PScm_E=p_BoBcm_B,
    ...
    def forward(self, q: Tensor, v: Tensor, u: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        inertia = InertialParameterConverter.pi_cm_to_drake_spatial_inertia_vector(
            self.pi_cm()
        )
...
def make_configuration_callback(
    expression: np.ndarray, q: np.ndarray
    ...
    return cast(
        ...
        drake_pytorch.sym_to_pytorch(
            expression, q, simplify_computation=DEFAULT_SIMPLIFIER
        )[0],
    )
...
class ContactTerms(Module):
    ...
    def __init__(
        ...
        if self.training:
            for candidate in collision_geometry_set.collision_candidates:
                if (
                    geometries[candidate[0]].learnable
                    or geometries[candidate[1]].learnable
                ):
        ...
        for geometry_index, geometry_pair in enumerate(collision_candidates):
            if geometries[geometry_pair[0]] > geometries[geometry_pair[1]]:
                collision_candidates[geometry_index] = (
                    geometry_pair[1],
                    geometry_pair[0],
                )
        ...
        if len(collision_candidates) < 1:
            raise ValueError(
                "Empty collision candiates. Do you have at least one learnable object geometry?"
            )

        self.geometry_rotations = make_configuration_callback(np.stack(rotations), q)
        ...
        for idx, friction in enumerate(coulomb_frictions):
            ...
            learnable = (
                (body.name() in learnable_body_dict)
                and (body != plant.world_body())
                and learnable_body_dict[body.name()].friction
            )
            self.friction_param_list.append(
                Parameter(
                    torch.tensor([friction.static_friction()]), requires_grad=learnable
                )
            )
    ...
    def extract_geometries_and_kinematics(
        ...
        for geometry_id in geometry_ids:
            geometry_pose = inspector.GetPoseInFrame(geometry_id).cast[Expression]()
    ...
    @staticmethod
    def relative_velocity_to_contact_jacobian(
        Jv_v_W_BcAc_F: Tensor, mu: Tensor
    ) -> Tensor:
        ...
        J_t = (mu.reshape(mu_shape) * torch.cat((J_x, J_y), dim=-1)).reshape(
...
class MultibodyTerms(Module):
    ...
    def scalars_and_meshes(self) -> Tuple[Dict[str, float], Dict[str, MeshSummary]]:
        """Generates summary statistics for inertial and geometric quantities."""
        scalars = {}
        meshes = {}
        bodies, all_body_ids = drake_utils.get_all_inertial_bodies(
            self.plant_diagram.plant, self.plant_diagram.model_ids
        )

        friction_coefficients = self.contact_terms.get_friction_coefficients()

        for body_pi, body_id, body in zip(
            self.lagrangian_terms.pi_cm(), all_body_ids, bodies
        ):
            ...
            for geometry_index in self.geometry_body_assignment[body_id]:
                ...
                if self.learnable_body_dict[body.name()].friction:
                    scalars[f"{body_id}_mu"] = friction_coefficients[
                        geometry_index
                    ].item()
                ...
                if geometry_mesh != None:
                    ...
                    diameters = vertices.max(dim=0).values - vertices.min(dim=0).values
    ...
    def forward(
    ...
    ) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, List, List, List]:
        ...
        phi, J, obj_pair_list, R_FW_list, mu_list = self.contact_terms(
            q, estimated_normals_W
        )
    ...
    def __init__(
        ...
        _, all_body_ids = drake_utils.get_all_bodies(plant, plant_diagram.model_ids)
        ...
        for geometry_index, geometry_id in enumerate(geometry_ids):
            ...
            geometry_body_assignment[geometry_body_identifier].append(geometry_index)
    ...
    def randomize_multibody_terms(self, inertia_int) -> None:
        ...
        for geometry in self.contact_terms.geometries:
            ...
            if isinstance(geometry, Plane):
                ...
                geometry.length_params = Parameter(
                    scale_factory(geometry.length_params), requires_grad=True
                )
            elif isinstance(geometry, Polygon):
                geometry.vertices_parameter = Parameter(
                    scale_factory(geometry.vertices_parameter), requires_grad=True
                )
                ...
                raise NotImplementedError(
                    "Can only randomize Box and Polygon "
        ...
        if (
            INERTIA_PARAM_OPTIONS[inertia_int] != "none"
            and INERTIA_PARAM_OPTIONS[inertia_int] != "masses"
        ):
            ...
            while keep_trying:
                ...
                for idx in range(n_bodies):
                    ...
                    if INERTIA_PARAM_OPTIONS[inertia_int] == "all":
                        ...
                        I_mat_pa = torch.tensor(
                            [[Ixx_pa, 0.0, 0.0], [0.0, Iyy_pa, 0.0], [0.0, 0.0, Izz_pa]]
                        )
