...
from dair_pll.integrator import Integrator, VelocityIntegrator, PartialStepCallback
...
class DeepLearnableSystem(System):
    ...
    def __init__(
        ...
        integrator = config.integrator_type(space, model, base_system.integrator.dt)
...
class DeepLearnableExperiment(SupervisedLearningExperiment, ABC):
    ...
    def get_learned_system(self, train_states: Tensor) -> System:
        deep_learnable_config = cast(
            DeepLearnableSystemConfig, self.config.learnable_config
        )

        return DeepLearnableSystem(
            self.get_base_system(), deep_learnable_config, train_states
        )
