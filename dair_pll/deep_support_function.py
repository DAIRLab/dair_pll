...
def get_mesh_summary_from_polygon(polygon) -> MeshSummary:
    ...
    vertices = (
        polygon.get_vertices(arbitrary_direction).squeeze(0).clone().cpu().detach()
    )
...
def extract_obj_from_mesh_summary(mesh_summary: MeshSummary) -> str:
    ...
    normals = extract_outward_normal_hyperplanes(
        mesh_summary.vertices.unsqueeze(0), mesh_summary.faces.unsqueeze(0)
    )[0].squeeze(0)
...
def extract_outward_normal_hyperplanes(vertices: Tensor, faces: Tensor):
    ...
    """
    batch_range = (
        torch.arange(vertices.shape[0]).unsqueeze(1).repeat((1, faces.shape[-2]))
    )
