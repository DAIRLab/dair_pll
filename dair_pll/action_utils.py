...
class Action:
    ...
    def random_uniform():
        return Action(
            ...
            np.random.uniform(-np.pi / 9.0, np.pi / 4.0),
            np.random.uniform(-np.pi / 4.0, np.pi / 4.0),
            np.random.uniform(-np.pi / 9.0, np.pi / 4.0),
        )
    ...
    def __post_init__(self):
        """Method to check validity of parameters."""
        self.finger_0_ra = np.clip(self.finger_0_ra, -np.pi / 4.0, np.pi / 4.0)
        self.finger_0_dec = np.clip(self.finger_0_dec, -np.pi / 9.0, np.pi / 4.0)
        self.finger_120_ra = np.clip(self.finger_120_ra, -np.pi / 4.0, np.pi / 4.0)
        self.finger_120_dec = np.clip(self.finger_120_dec, -np.pi / 9.0, np.pi / 4.0)
