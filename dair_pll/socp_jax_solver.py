...
def solve_single_pgd(J: jnp.ndarray, q: jnp.ndarray, eps: float, max_iter: int) -> jnp.ndarray:
    """
    Solves a single SOCP problem instance.
    """
    dim_q = q.shape[0]
    ...
    v = jax.random.normal(jax.random.PRNGKey(0), (dim_q, 1), dtype=jnp.float32)
    ...
    L = jnp.matmul(jnp.matmul(v.T, H), v).squeeze()
    ...
    def scan_body(state: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, float], _):
        ...
        # Compute gradient: grad = H * y + q
        grad = jnp.matmul(H, y) + q
...
def accelerated_pgd_socp_sappy_jax(J: jnp.ndarray, q: jnp.ndarray, eps: float, max_iter: int = 100) -> jnp.ndarray:
    """
    Solves SOCP problems using vmap for batching.
    """
    # Use vmap to parallelize the single-instance solver across the batch
