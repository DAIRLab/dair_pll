...
def qjac(q, p):
    # Returns the jacobian of the coordinates p w.r.t. the elements of q
    # https://math.stackexchange.com/questions/2713061/jacobian-of-a-quaternion-rotation-wrt-the-quaternion # noqa
    batch_n = q.shape[0]

    quat = qmat(qmul(q, p)).bmm(inv_mat(batch_n)) + pmat(qmul(p, qinv(q)))
...
def qfix_torch(q):
    ...
    result = q.clone()
    dot_products = (q[1:] * q[:-1]).sum(dim=2)
    mask = dot_products < 0
...
def quaternion_to_rotmat_vec(q):
    ...
    qk = q[:, 3:4]
    r1 = torch.cat(
        (1. - 2 * (qj ** 2 + qk ** 2), 2 * (qi * qj - qk * qr),
         2 * (qi * qk + qj * qr)), dim=1)  # noqa
    r2 = torch.cat(
        (2 * (qi * qj + qk * qr), 1. - 2 * (qi ** 2 + qk ** 2),
         2 * (qj * qk - qi * qr)), dim=1)  # noqa
    r3 = torch.cat(
        (2 * (qi * qk - qj * qr), 2 * (qj * qk + qi * qr),
         1. - 2 * (qi ** 2 + qj ** 2)), dim=1)  # noqa
...
def quat_to_rvec(q):
    assert q.size()[-1] == 4
    cos = q[:, 0:1]
    sin = q[:, 1:].norm(dim=1).reshape(-1, 1)
    theta = torch.atan2(sin, cos) * 2.
    # cot = 1 / sin
    # cot[cot == float('Inf')] = 0.
    # e = theta * (q[:, 1:] * cot)
    sin[sin == 0.0] = float('Inf')
    e = theta * (q[:, 1:] / sin)
    return e


def quat_to_rvec_gradsafe(q):
    # if len(q.shape) > 2:
    #    pdb.set_trace()
    assert q.size()[-1] == 4
    cos = q[..., 0:1]
    sin = q[..., 1:].norm(dim=-1).unsqueeze(-1)  # .reshape(-1, 1)
    theta = torch.atan2(sin, cos) * 2.
    # cot = 1 / sin
    # cot[cot == float('Inf')] = 0.
    # e = theta * (q[:, 1:] * cot)
    # pdb.set_trace()
    div = torch.zeros_like(sin)
    div[sin == 0.0] += 1e8
    div[sin > 0.0] += sin[sin > 0.0]
    e = theta * (q[..., 1:] / div)
    return e
...
def rvec_to_quat(e):
    ...
    original_shape = list(e.size())
    original_shape[-1] = 4
    e = e.reshape(-1, 3)

    theta = e.norm(dim=1).reshape(-1, 1)
    w = torch.cos(0.5 * theta).reshape(-1, 1)
    xyz = 0.5 * sinc(0.5 * theta / np.pi) * e
...
def sinc(x):
    ...
    x[null] *= 0.
    x[null] += 1.
    x[notnull] = torch.sin(x[notnull] * np.pi) / (x[notnull] * np.pi)
    return x


if __name__ == '__main__':
    quats = torch.rand((2, 8, 4))
    quats = quats / quats.norm(dim=-1).unsqueeze(-1)
    print(quat_to_rvec_gradsafe(quats).shape)
