    ...
    for N_train in datasizes:
        ...
        if N_runs > 0:
            ...
            for N_run in range(N_runs):
                runfile = file_utils.sweep_summary_file(study_name, N_train, N_run)
...
def log_gaussian_band_values(
    runs: List[Dict], param: str, param2: Optional[str] = None
    ...
    if param2 is None:
        ...
        v = torch.tensor([tt(d[param]).mean() - tt(d[param2]).mean() for d in runs])
    ...
    return (torch.exp(l - r).item(), torch.exp(l).item(), torch.exp(l + r).item())


def query_sweep(
    sweep: Dict[int, List], func: Callable, param: str, param2: Optional[str] = None
...
def plot_sweep_comparison(plot_name: str, study_names: List[str]) -> None:
    ...
    for study_name in study_names:
        ...
        for i in range(len(comparison_chain)):
            query = query_sweep(
                sweep,
                log_gaussian_band_values,
                comparison_chain[i][0],
                comparison_chain[i][1],
            )
            bands.append(query)
    ...
    for i in range(len(comparison_chain)):
        ...
        ps = PlotStyler()
        j = 0
        for x_low, x, x_high, y_low, y, y_high in zip(
            x_lows, xs, x_highs, y_lows, ys, y_highs
        ):
...
if __name__ == "__main__":
    ...
    if TEST:
        ...
        study_names = [f"mujoco_cube_{stiffness}_sweep_test" for stiffness in [300]]
    else:
        plot_name = "mujoco_cube"
        study_names = [f"mujoco_cube_{stiffness}" for stiffness in [100, 300, 2500]]
