...
class DeepRecurrentModel(DeepLearnableModel):
    ...
    def __init__(
        ...
        if encode:
            self.encoder = _mlp(
                in_size, hidden_size, hidden_size, layers // 2, nonlinearity
            )
        else:
            self.encoder = lambda x: x
        self.decoder = _mlp(
            hidden_size, hidden_size, out_size, layers - (layers // 2), nonlinearity
        )
...
def _mlp(
    in_size: int, hidden_size: int, out_size: int, layers: int, nonlinearity: Module
) -> Module:
    modules = []
    if layers == 0:
...
class ZeroModel(DeepLearnableModel):
    ...
    def sequential_eval(self, x: Tensor, carry: Tensor) -> Tensor:
        # x is B x L x N
        return self.dummy_param * torch.zeros((x.shape[0], 1, self.out_size)), carry
