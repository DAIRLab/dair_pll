...
def load_results_from_experiment(exp_name):
    ...
    def compile_experiment_config_dict(beginning_lines):
        ...
        experiment_config["system"] = (
            lns[1].split(": ")[1][:-1].replace(" ", "").replace(".", "")
        )
        experiment_config["source"] = (
            lns[2].split(": ")[1][:-1].replace(" ", "").replace(".", "")
        )
        experiment_config["inertia_mode"] = (
            lns[9].split(": ")[1][:-1].replace(" ", "").replace(".", "")
        )
        experiment_config["loss_type"] = (
            "ContactNets" if "True" in lns[3] else "prediction"
        )
        experiment_config["geometry_type"] = "box" if "True" in lns[4] else "mesh"
        experiment_config["initial_urdf"] = "correct" if "True" in lns[10] else "wrong"
        ...
        config_line = 11
        while "n_pop" not in lns[config_line]:
            config_line += 1

        # get the timestep
        experiment_config["timestep"] = float(
            lns[config_line].split("dt=")[1].split(",")[0]
        )
        ...
        if experiment_config["source"] == "simulation":
            ...
            while "n_import" not in lns[config_line]:
                config_line += 1
    ...
    def compile_datasets(beginning_lines):
        ...
        i = 11
        while "indices" not in beginning_lines[i]:
            i += 1
...
if __name__ == "__main__":
    ...
    exp_config, dataset_indices, scalar_list, stat_list = load_results_from_experiment(
        "t19"
    )
