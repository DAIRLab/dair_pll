...
CN_METHODS_ONLY = ["Vimp", "Vimp RP"]  #'VimpI', 'VimpI RP',
...
METRICS = {
    "model_loss_mean": {
        "label": "Loss",
        "scaling": 1.0,
        "yformat": {"elbow": "%.0f", "cube": "%.0f", "asymmetric": "%.0f"},
        "ylims": {
            "elbow": [None, None],
            "cube": [None, None],
    ...
    "model_angle_int_traj": {
        ...
        "scaling": 180 / np.pi,
        "yformat": {"elbow": "%.0f", "cube": "%.0f", "asymmetric": "%.0f"},
        "ylims": {"elbow": [0.0, 140], "cube": [0.0, 140], "asymmetric": [0.0, 140]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
        "log": False,
    },
...
PARAMETER_ERRORS = {
    GEOMETRY_PARAMETER_ERROR: {
        "label": "Geometry parameter error [m]",
        "scaling": 1.0,
        "yformat": {"elbow": "%.3f", "cube": "%.3f", "asymmetric": "%.3f"},
        "ylims": {"elbow": [0.0, None], "cube": [0.0, None], "asymmetric": [0.0, None]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
        "log": False,
    },
    VERTEX_ERROR: {
        "label": "Average vertex location error [m]",
        "scaling": 1.0,
        "yformat": {"elbow": "%.2f", "cube": "%.2f", "asymmetric": "%.2f"},
        "ylims": {"elbow": [0.0, None], "cube": [0.0, None], "asymmetric": [0.0, None]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
    ...
    VOLUME_ERROR: {
        ...
        "scaling": 1.0,
        "yformat": {"elbow": "%.2f", "cube": "%.2f", "asymmetric": "%.2f"},
        "ylims": {"elbow": [0.0, 0.52], "cube": [0.0, 0.52], "asymmetric": [0.0, 0.52]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
        "log": False,
    },
    FRICTION_PARAMETER_ERROR: {
        "label": "Friction error",
        "scaling": 1.0,
        "yformat": {"elbow": "%.1f", "cube": "%.2f", "asymmetric": "%.2f"},
        "ylims": {"elbow": [0.0, 0.85], "cube": [0.0, 0.85], "asymmetric": [0.0, 0.85]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
        "log": False,
    },
    INERTIA_PARAMETER_ERROR: {
        "label": "Inertia parameter error",
        "scaling": 1.0,
        "yformat": {"elbow": "%.0f", "cube": "%.2f", "asymmetric": "%.2f"},
        "ylims": {"elbow": [0.1, 300], "cube": [0.1, 300], "asymmetric": [0.1, 300]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
        "log": True,
    },
    "init_"
    ...
    + INERTIA_PARAMETER_ERROR: {
        ...
        "scaling": 1.0,
        "yformat": {"elbow": "%.0f", "cube": "%.2f", "asymmetric": "%.2f"},
        "ylims": {"elbow": [0.1, None], "cube": [0.1, None], "asymmetric": [0.1, None]},
        "legend_loc": {"elbow": "best", "cube": "best", "asymmetric": "best"},
        "log": True,
    },
}

ALL_PARAMETER_METRICS = [
    GEOMETRY_PARAMETER_ERROR,
    FRICTION_PARAMETER_ERROR,
    INERTIA_PARAMETER_ERROR,
    VERTEX_ERROR,
...
GRAVITY_FRACTION_DICT = {0.0: [], 0.5: [], 1.0: [], 1.5: [], 2.0: []}
...
def calculate_error_vertices(vertices_learned: Tensor, vertices_true: Tensor) -> Tensor:
    ...
    halfspaces_true = torch.cat(
        [normal_true.squeeze(), -extent_true.squeeze().unsqueeze(-1)], dim=1
    )

    halfspaces_learned = torch.cat(
        [normal_learned.squeeze(), -extent_learned.squeeze().unsqueeze(-1)], dim=1
    )

    intersection_halfspaces = torch.cat(
        [halfspaces_true, halfspaces_learned], dim=0
    ).numpy()
    ...
    if interior_point_gap > 0.0:
        ...
        intersection_volume = ConvexHull(
            intersection_halfspace_convex.intersections
        ).volume
...
def get_empty_experiment_dict_by_experiment(experiment):
    ...
    for method in empty_dict_per_experiment.keys():
        ...
        empty_dict_per_experiment[method].update({N_RUNS: deepcopy(DATASET_SIZE_DICT)})
        for metric in METRICS.keys():
            empty_dict_per_experiment[method][metric] = deepcopy(DATASET_SIZE_DICT)
...
def get_empty_gravity_experiment_dict_by_experiment(experiment):
    ...
    for method in empty_dict_per_experiment.keys():
        ...
        for metric in METRICS.keys():
            empty_dict_per_experiment[method][metric] = deepcopy(GRAVITY_FRACTION_DICT)
...
def fill_exp_dict_with_single_run_data(
    run_dict, sweep_instance, exp_dict, gravity=False
):
    ...
    for result_metric in run_dict["results"].keys():
        new_key = result_metric[5:] if result_metric[:5] == "test_" else result_metric
...
def convert_lists_to_t_conf_dict(exp_dict, sweep_instance):
    ...
    for method in METHOD_RESULTS.keys():
        ...
        for quantity in exp_dict[method].keys():
            ...
            exp_dict[method][N_RUNS][sweep_instance] = max(
                len(vals), exp_dict[method][N_RUNS][sweep_instance]
            )
...
def get_scatter_plottable_values(
    exp_dict, metric, method, metric_lookup, gravity=False
):
    try:
        data_dict = exp_dict[method][metric]
...
def format_plot(ax, fig, metric, metric_lookup, experiment, gravity=False):
    ...
    ax.set_ylim(
        bottom=metric_lookup[metric]["ylims"][system][0],
    ...
    if (
        FORCE_ALL_PLOT_LABELS
        or (experiment == "elbow" and not gravity)
        or metric in [INERTIA_PARAMETER_ERROR, FRICTION_PARAMETER_ERROR]
    ):
...
def format_scatter_plot(
    ax, fig, metric, metric_lookup, experiment, exponent=0, correlations=False
):
    system = SYSTEM_BY_EXPERIMENT[experiment.split("_gravity")[0]]
...
def calculate_geometry_error(run_dict, experiment, init=False):
    ...
    for body in CORRECT_PARAMETERS_BY_SYSTEM_AND_BODY[system].keys():
        ...
        ground_truth_verts = CORRECT_PARAMETERS_BY_SYSTEM_AND_BODY[system][body][
            "vertices"
        ]
        ...
        volume_err += calculate_vertex_position_error(ground_truth_verts, learned_verts)

        ground_truth = get_single_body_correct_geometry_array(system, body)

        learned = np.array(
            [
                body_dict["diameter_x"],
                body_dict["diameter_y"],
                body_dict["diameter_z"],
                body_dict["center_x"],
                body_dict["center_y"],
                body_dict["center_z"],
            ]
...
def calculate_inertia_error(run_dict, experiment, init=False):
    ...
    # Insert this error into the results dictionary.
    run_dict["results"].update({prefix + INERTIA_PARAMETER_ERROR: inertia_error})
    return run_dict
...
def calculate_friction_error(run_dict, experiment, init=False):
    ...
    # Insert this error into the results dictionary.
    run_dict["results"].update({prefix + FRICTION_PARAMETER_ERROR: friction_error})
    return run_dict
...
def include_fixed_horizon_post_stats(run_dict, experiment):
    ...
    for post_metric in FIXED_HORIZON_METRICS_BY_EXPERIMENT[experiment]:
        run_dict["results"].update({post_metric: params_dict[f"test_{post_metric}"]})
    return run_dict
...
def do_run_num_plot(exp_dict, experiment, gravity=False):
    ...
    for method in METHOD_RESULTS.keys():
        ...
        ax.plot(
            xs, ys, label=method, linewidth=5, color=METHOD_RESULTS[method]["color"]
        )
...
for experiment in results.keys():
    ...
    for exponent_str in data_sweep.keys():
        ...
        for run_name, run_dict in data_sweep[exponent_str].items():
            if run_name[2:4] in RUN_NUMBERS_TO_SKIP:
                if not sent_warning:
                    print(
                        f"WARNING: Skipping any run numbers in "
            ...
            if get_method_name_by_run_dict(run_dict) == "dummy":
                continue
            run_dict = include_fixed_horizon_post_stats(run_dict, experiment)
            exp_dict = fill_exp_dict_with_single_run_data(run_dict, exponent, exp_dict)
    ...
    for parameter_metric in PARAMETER_METRICS_BY_EXPERIMENT[experiment]:
        ...
        for exponent in range(8):
            ...
            fig_path = op.join(
                OUTPUT_DIR, f"{experiment}_comp_{exponent}_{parameter_metric}.png"
            )
            fig.savefig(fig_path, dpi=100)
            plt.close()
    ...
    for parameter_metric in PARAMETER_METRICS_BY_EXPERIMENT[experiment]:
        ...
        for method in METHOD_RESULTS.keys():
            ...
            print(
                f"{experiment}, {parameter_metric}, {method}, correlation: {corr.statistic:.4f}, {corr.pvalue:.4f}"
            )

        format_scatter_plot(
            ax, fig, parameter_metric, PARAMETER_ERRORS, experiment, correlations=True
        )

        fig_path = op.join(OUTPUT_DIR, f"{experiment}_corr_{parameter_metric}.png")
        fig.savefig(fig_path, dpi=100)
        plt.close()
...
for experiment in results.keys():
    ...
    for grav_frac in gravity_sweep.keys():
        ...
        for run_name, run_dict in gravity_sweep[str(grav_frac)].items():
            ...
            run_dict = convert_parameters_to_errors(run_dict, experiment, gravity=True)
            if get_method_name_by_run_dict(run_dict) == "dummy":
                continue
    ...
    for metric in METRICS.keys():
        ...
        for method in METHOD_RESULTS.keys():
            ...
            ax.fill_between(
                xs, lowers, uppers, alpha=0.3, color=METHOD_RESULTS[method]["color"]
            )

        format_plot(ax, fig, metric, METRICS, f"{experiment}_gravity", gravity=True)
    ...
    for parameter_metric in PARAMETER_METRICS_BY_EXPERIMENT[exp_key]:
        ...
        for method in METHOD_RESULTS.keys():
            xs, ys, lowers, uppers = get_plottable_values(
                exp_dict, parameter_metric, method, PARAMETER_ERRORS, gravity=True
            )
